# Create the generation module with fixed string formatting
generation_content = '''"""
...
class MockLLMProvider:
    """Mock LLM provider for testing when no real LLM is available"""
    ...
    def generate(self, prompt: str, max_tokens: int = None, temperature: float = None) -> str:
        ...
        self.logger.info("Using mock LLM provider - generating sample response")
        
        # Extract context sources for citation
        import re
        citations = re.findall(r'\\[(\\d+)\\]', prompt)
        
        response = "Based on the provided context, I can provide the following information:\\n\\n"
        
        if citations:
            response += f"The documents provide relevant information about your query. "
            response += f"According to the sources [{']['.join(citations[:3])}], "
            response += "the key points include important insights that directly address your question. "
            response += f"Additional details can be found in source [1] which provides comprehensive coverage of the topic."
        else:
            response += "The provided context contains relevant information that helps answer your question. "
            response += "The sources provide detailed insights on this topic."
...
class ResponseGenerator:
    ...
    def __init__(self):
        ...
        # Initialize providers based on configuration and availability
        try:
            ...
            self.logger.warning(f"Could not initialize LLM provider: {e}. Using mock provider.")
    ...
    def _calculate_confidence(self, response: str, retrieval_results: List[Dict[str, Any]]) -> float:
        ...
        source_coverage = min(citations_count / len(retrieval_results), 1.0) if retrieval_results else 0.0
